...
function calcTotals(
  items: { description: string; unit?: string | null; quantity: number; rate: number }[],
  tax: number,
  discount: number
) {
...
function addBalanceDue<T extends { grandTotal: unknown; receivedAmount: unknown }>(invoice: T) {
...
async function findInvoice(organizationId: number, invoiceId: number) {
...
function pickOptionalFields(payload: ICreateInvoice | IUpdateInvoice) {
  return {
...
const updateInvoice = async (
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (payload.receivedAmount !== undefined) updateData.receivedAmount = payload.receivedAmount;
    ...
    if (payload.items !== undefined) {
      ...
      updateData.amountInWords = amountInWords(grandTotal);
